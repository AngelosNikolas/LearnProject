...
In the question response data set the relevant columns are the question number and the correct column for the analysis conducted. These two columns have been checked and they do not indicate any problems.

### QUestion response data constuction for analysis
For the question response data set 3 data frames are constructed filtered by the correct answers for the questions for each week respectively. This will later utilized to compare performance in terms of success. Furthermore, the best performing week will be analyzed further. The results will be stored inside a new set to visualized in the analysis section.
...
## Enrollements
The analysis starts with the enrollment data set. The first goal is to make a comparison regarding the enrolled learners throughout the all runs.
```{r , fig.width=6, fig.height=2} 
EnrollCountPlot
```
The plot depicts all enrollments made in each run the most popular one being the first 14840 enrolled learners after that comes the second run with 6488. The rest runs are similar in terms of people enrolled indicating that the most popular one was the first.
Next, the fully participation percentages can be computed meaning how many enrolled learners fully participated throughout the course. In the table below are stored the calculated percentages and then are visualized using a pie chart. 
...
Taking that 12% subset for the learner from the first run some investigation took place regarding genders, education and employment.
```{r , fig.width=6, fig.height=2} 
Run1Gender
Education_Plot
Employment_Plot
...
### Enrollements conclussion 
. The most popular run was the first one that also had the most high completion rate regarding the full course, perhaps indicating it captured the learners interest the most. Unfortunately, the data set contains a significant amount of unknown values making further investigation highly speculative. It would be advisable to keep a better track about the learners in future runs because their background could assist in improving the course for learners that are interested and share similar characteristics.

##Question response
For the analysis only the runs 1, 2 and 3 will be analyzed. The reasons for this decision is the difference that was noticed in the enrollment analysis between the runs. Furthermore, the first run is a very indicative for the performance of the course, on the second run and onward differences in the steps were noticed. Indicating changes that occurred in the course structure after the first run. This will be further analyzed with the step activity investigation. The third run is also important because video stats are introduced which are a part of the analysis.
...
Again as expected the completion rate is significant with minor changes across runs. Although, the type are not equal is size. The following table depicts the step number counts for the first run:
```{r}
table1
```
...
The video stats data set contains various information about the videos viewed throughout the course by the learners. The first investigation focus on what affects viewership. In that regard, a plot created depicted total views and video duration:
```{r}
VideoPlot1
```
The plot seems to indicate that when the duration is roughly between 200 and 350 seconds the views on average are at 600 . An outlier is noticeable in a very short video but no solid conclusions can be discussed. Although, and assumption can be made could about the suggested video lengths. The videos shouldn't be too short or too long, this could negatively affect engagement levels by the learners.
